function [qpar, negloglike, bic, nlike, aic, endVals] = fit_fun_initV(stats, fit_fun, initpar, lb, ub)    
% % fit_fun %
...
if ~exist('lb','var')
    [qpar, negloglike, exitflag] = fminsearch(func_handle, initpar, op, [dat],initVals);    
else
    [qpar, negloglike, exitflag] = fmincon(func_handle, initpar, [], [], [], [], lb, ub, [], op, [dat],initVals);
end
if exitflag==0
...
[~, ~, V1, V2] = func_handle(qpar,dat,initVals);
V_endvals = [V1(end),V2(end)];
endVals{1} = V_endvals;
