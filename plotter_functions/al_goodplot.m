function [h, mu, sigma, q, notch] = al_goodplot(x, pos, boxw, col, type, bw, p, jitw, DistFaceAlpha, boxFaceAlpha)
...
if sum(isnan(x))>0; x(isnan(x)) = []; end
...
if nargin<7 || isempty(p)  
    p=std(x(:),'omitnan')/1000;
end
if nargin<8 || isempty(jitw)  
    jitw=0;
end
if nargin<9 || isempty(DistFaceAlpha)
    DistFaceAlpha = 0.3;
end
if nargin<10 || isempty(boxFaceAlpha)
    boxFaceAlpha = 0.5;
end
% boxFaceAlpha = 0.5;
% DistFaceAlpha = 0.3;
% SDevFaceAlpha = 0.35;
SDevFaceAlpha = DistFaceAlpha + 0.05;
...
%             scatter(pos(i)*ones(size(x(:,i)))+jitw*(rand(size(x(:,i)))-0.5),x(:,i),10,col(i,:),'filled');
            hold on
            patch([pos(i)-f fliplr(pos(i)+f)], [uc fliplr(uc)], 0.97*col(i,:),'edgecolor','none','facealpha',DistFaceAlpha);
            ...
            patch([pos(i)-boxw(i)/8 pos(i)+boxw(i)/8 pos(i)+boxw(i)/8 pos(i)-boxw(i)/8 pos(i)-boxw(i)/8], [mu(i)-sigma(i) mu(i)-sigma(i) mu(i)+sigma(i) mu(i)+sigma(i) mu(i)-sigma(i)], col(i,:),'edgecolor','none','facealpha',SDevFaceAlpha,'HandleVisibility','off')
            plot([pos(i)-boxw(i)/4 pos(i)+boxw(i)/4], [q(3,i) q(3,i)],'color',col(i,:)/2,'linewidth',1,'HandleVisibility','off')
            plot(pos(i), mu(i),mean_marker,'color',col(i,:)/2,'MarkerSize',markerSz,'linewidth',1,'HandleVisibility','off')
...
%             scatter((pos(i)-boxw(i)/40)*ones(size(x(:,i)))-jitw*0.25*rand(size(x(:,i))),x(:,i),10,col(i,:),'filled');
            hold on
            patch(pos(i)-f, uc, 0.97*col(i,:),'edgecolor','none','facealpha',DistFaceAlpha,'HandleVisibility','off');
            patch([pos(i) pos(i)-boxw(i)/2 pos(i)-boxw(i)/2 pos(i)-boxw(i)/4 pos(i)-boxw(i)/2 pos(i)-boxw(i)/2 pos(i) pos(i)], [q(2,i) q(2,i) notch(1,i) q(3,i) notch(2,i) q(4,i) q(4,i) q(2,i)], 0.97*col(i,:),'edgecolor','none','facealpha',boxFaceAlpha,'HandleVisibility','off')
%             patch([pos(i)-boxw(i)/8 pos(i) pos(i) pos(i)-boxw(i)/8 pos(i)-boxw(i)/8], [mu(i)-sigma(i) mu(i)-sigma(i) mu(i)+sigma(i) mu(i)+sigma(i) mu(i)-sigma(i)], col(i,:),'edgecolor','none','facealpha',SDevFaceAlpha,'HandleVisibility','off')
            plot([pos(i)-boxw(i)/4 pos(i)], [q(3,i) q(3,i)],'color',col(i,:)/2,'linewidth',1.5,'LineStyle','-','Marker','none','HandleVisibility','off')
            plot(pos(i)-boxw(i)/2, mu(i),mean_marker,'color',col(i,:)/2,'MarkerSize',markerSz,'linewidth',1,'HandleVisibility','off')
